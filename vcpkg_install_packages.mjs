...
async function vcpkg_install(ports, triplets) {
    ...
    const buildtreesDir = process.env.VCPKG_BUILDTREES_ROOT || '';
    if (buildtreesDir && (await fs.pathExists(buildtreesDir))) {
        options.push(`--x-buildtrees-root=${buildtreesDir}`);
    }
    let list = [];
...
for (const platform of platforms) {
    ...
    if (stdout.indexOf('Binary cache submission failed:') >= 0) {
        console.error(`At least one binary cache submission failed.`);
        process.exit(1);
    }
}
